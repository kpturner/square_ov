...
const ovSelectionList = computed(() => {
  return ovMasters.value.map((ov) => {
    return {
      ...
      title:
        ov.ovType === 'ra'
          ? `${ov.number}: ${ov.lodgeName.replace(' C', ' Chapter').replace('C of', 'Chapter of')} No. ${ov.lodgeNumber}`
...
const selectedOVName = computed(() => {
  ...
  if (selectedMasterOV.value.ovType === 'ra') {
    return `${selectedMasterOV.value.lodgeName.trim().replace(' C', ' Chapter').replace('C of', 'Chapter of')} No. ${selectedMasterOV.value.lodgeNumber}`;
  }
